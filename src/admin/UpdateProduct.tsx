...
const UpdateProduct = () => {
  ...
  const fetchCurrentProductData = async (id: string) => {
    try {
      const response = await fetch(`http://localhost:3000/products/${id}`);
      const currentProductData = await response.json();
      reset(currentProductData.data);
    } catch (err) {
      console.error(err);
    }
  };
  ...
  const onUpdate: SubmitHandler<TInput> = async (data: TInput) => {
    const dataToUpdate = { ...data, category: data.category._id };
    updateProduct(id as string, dataToUpdate);
  };
